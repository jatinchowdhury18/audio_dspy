...
def checkCoefs(tester, b, a, b_exp, a_exp):
    for n in range(len(b)):
        tester.assertTrue(np.abs (b[n] - b_exp[n]) < _tolerance_, 'Expected: b[{}] = {}. Actual: b[{}] = {}'.format (n, b_exp[n], n, b[n]))
    
    for n in range(len(a)):
        tester.assertTrue(np.abs (a[n] - a_exp[n]) < _tolerance_, 'Expected: a[{}] = {}. Actual: a[{}] = {}'.format (n, a_exp[n], n, a[n]))
...
class TestEQ(TestCase):
    ...
    def test_eq_LPFN(self):
        ...
        for order in range(1, 20):
            ...
            exp_diff_dB = -6 * order
            diff_dB = 20 * np.log10 (np.abs (H[freq2_ind])) - 20 * np.log10 (np.abs (H[freq1_ind]))
            self.assertTrue (np.abs (diff_dB - exp_diff_dB) < _tolerance_*1000, "Incorrect slope! Expected: {}, Actual: {}".format (exp_diff_dB, diff_dB))
    ...
    def test_eq_HPFN(self):
        ...
        for order in range(1, 20):
            ...
            exp_diff_dB = -6 * order
            diff_dB = 20 * np.log10 (np.abs (H[freq2_ind])) - 20 * np.log10 (np.abs (H[freq1_ind]))
            self.assertTrue (np.abs (diff_dB - exp_diff_dB) < _tolerance_*5000, "Incorrect slope! Expected: {}, Actual: {}".format (exp_diff_dB, diff_dB))
