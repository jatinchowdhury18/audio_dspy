...
class TestTFs(TestCase):
    ...
    def test_tf2linphase(self):
        ...
        self.assertTrue (np.max (diffs) < _tolerance_, 'Linear Phase IR is not symmetric! {}'.format (np.max (diffs)))
    ...
    def test_tf2minphase(self):
        ...
        self.assertTrue (min_delay[0] <= orig_delay[0], 'Minimum phase IR does not have minimum group delay!')
