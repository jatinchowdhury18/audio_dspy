...
class TestProny(TestCase):
    ...
    def test_warp_roots(self):
        ...
        test_angle = np.arctan2 ((1 - _rho_**2) * np.sin (pole_freq_uw), (1 + _rho_**2) * np.cos (pole_freq_uw) - 2 * _rho_)

        self.assertTrue (np.abs (pole_freq_w - test_angle) < _tolerance_, 'Expected: {}, Actual: {}'.format (test_angle, pole_freq_w))
